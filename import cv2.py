...
def preprocess_image(image_path, crop_area=(500, 1000), threshold_value=100):
    """
...
def find_safe_landing_point(binary_image, initial_landing_pixel, max_search_radius=50):
    """
    Find the nearest safe landing point to the initial landing pixel.
    """
    if binary_image[initial_landing_pixel[0], initial_landing_pixel[1]] == 255:
        return initial_landing_pixel  # Already safe

    # Search for the nearest safe point within the specified radius
    height, width = binary_image.shape
    for radius in range(1, max_search_radius + 1):
        for y in range(max(0, initial_landing_pixel[0] - radius), min(height, initial_landing_pixel[0] + radius + 1)):
            for x in range(max(0, initial_landing_pixel[1] - radius), min(width, initial_landing_pixel[1] + radius + 1)):
                if binary_image[y, x] == 255:  # Found a safe point
                    return (y, x)

    raise ValueError("No safe landing point found within the search radius.")

def find_safe_waypoints(binary_image, landing_pixel, num_points=10, step_distance=10):
    """
    Finds safe waypoints by checking for free points (white areas in binary image).
    """
    height, width = binary_image.shape
        ...
        for y in range(landing_pixel[0] - step_distance, landing_pixel[0] + step_distance + 1):
            for x in range(landing_pixel[1] - step_distance, landing_pixel[1] + step_distance + 1):
                if 0 <= x < width and 0 <= y < height and binary_image[y, x] == 255:
                    ...
                    landing_pixel = (y, x)  # Move to new waypoint
...
def pixel_to_coordinates(pixel, binary_image, landing_coordinates=LANDING_COORDINATES):
    """
    Convert pixel coordinates to latitude and longitude based on the image scale.
    """
    image_height, image_width = binary_image.shape
    lat_range = 0.001  # Example scale for latitude
    lon_range = 0.001  # Example scale for longitude

    # Calculate lat/lon based on pixel position
...
def main():
    image_path = r"C:\SIRIUS\greyscale_image.jpg"  # Provide correct image path
    binary_image = preprocess_image(image_path)

    # Initial landing point (center of the cropped image)
    initial_landing_pixel = (binary_image.shape[0] // 2, binary_image.shape[1] // 2)

    # Find a safe landing point
    landing_pixel = find_safe_landing_point(binary_image, initial_landing_pixel)

    # Find safe waypoints
    safe_waypoints = find_safe_waypoints(binary_image, landing_pixel, num_points=10, step_distance=10)
    ...
    plt.plot(safe_waypoints[:, 1], safe_waypoints[:, 0], 'r-', linewidth=2)  # Path
    plt.scatter(safe_waypoints[:, 1], safe_waypoints[:, 0], c='blue', marker='o')  # Waypoints

    # Annotate waypoints with lat/lon
